...
bool Direct3D_Initialize(HWND hWnd)
{
	...
	*/
	...
	depthStencilDesc.DepthFunc = D3D11_COMPARISON_LESS; // 深度比較関数を設定
	depthStencilDesc.StencilEnable = FALSE; // ステンシルを無効にする
...
void Direct3D_Clear()
{
	...
	g_pDeviceContext->OMSetRenderTargets(1, &g_pRenderTargetView, g_pDepthStencilView); // レンダーターゲットビューとデプスステンシルビューを設定
}
...
bool configureBackBuffer()
{
	...
	g_pDeviceContext->RSSetViewports(1, &g_Viewport); // ビューポートを設定
	return true;
}
