...
//プレイヤーステータス
float moveSpeed = 0.005f;			//移動速度（いどうそくど）
float maxMoveSpeed = 1.0f;			//最大移動速度（さいだいいどうそくど）
float maxGravity = -0.25f;			//最大落下速度（さいだいらっかそくど）
float jumpPower = 0.175f;			//ジャンプ力（じゃんぷりょく）
bool isGround = false;				//接地判定（せっちはんてい）

//キーボード定義（ていぎ）
//移動（いどう）
static const auto UpKey = KK_W;			//前（まえ）
static const auto RightKey = KK_D;		//右（みぎ）
static const auto DownKey = KK_S;		//後（うしろ）
static const auto LeftKey = KK_A;		//左（ひだり）
//行動（こうどう）
static const auto JumpKey = KK_SPACE;	//ジャンプ
static const auto ActionKey = KK_F;		//アクション
static const auto ChangeKey = KK_F;		//影変身（かげへんしん）
//その他（そのた）
static const auto ResetKey = KK_R;		//リセット
static const auto MenuKey = KK_ESCAPE;	//ポーズメニュー
...
void Player3D_Move()
{
	// 前フレームの入力をリセット（キーを離したときに以前の入力が残らないようにする。これがないと一度押したらその方向に動き続ける）
